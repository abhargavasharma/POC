IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.spAgentDashboardSearch'))
   exec('CREATE PROCEDURE [dbo].[spAgentDashboardSearch] AS BEGIN SET NOCOUNT ON; END')
GO
...
--Get the last pipeline status change for the user
       ...
       FROM    PolicyInteraction p
        ...
        SELECT pac.PolicyId, pac.LastTouchedByTS, pol.QuoteReference, pol.Premium, pol.PolicyProgressId, pol.BrandId, polProg.[Description] AS LastPipelineStatusDescription,
...
--Get last access of quote with time range filtering and pagination enabled
        ...
        SELECT pd.PolicyId, LastTouchedByTS, QuoteReference, Premium, LastPipelineStatusDescription, PolicyProgressId, LastPipelineStatusTS, TotalRecords, BrandId
...
, Owners AS (
SELECT  ap.PolicyId, p.Firstname + ' ' + p.Surname AS OwnerFullName, po.PartyId as OwnerPartyId,  p.ExternalCustomerReference as OwnerExternalCustomerReference
...
SELECT  
       ...
       , o.OwnerFullName
       , o.OwnerPartyId
	   , o.OwnerExternalCustomerReference
