(function (module) {
    ...
    function talBasicInfoFormController($log, $scope, ngDialog, talCustomerPortalApiService, talFormModelStateService,
        ...
        this.useCalcResults = function (calcResults) {
            ...
            $log.debug(calcResults.results);
            $log.debug(calcResults.answers);
            $log.debug(calcResults.assumptions);

            talCustomerPortalApiService.setCalcResultsForRisk(calcResults);
        };

        $scope.$on('ocLazyLoad.moduleLoaded', function(e, module) {
            $log.debug('ocLazyLoad module loaded', module);
            if(module === 'coverCalculator'){
                ctrl.isCoverCalculatorEnabled = true;
            }
        });
