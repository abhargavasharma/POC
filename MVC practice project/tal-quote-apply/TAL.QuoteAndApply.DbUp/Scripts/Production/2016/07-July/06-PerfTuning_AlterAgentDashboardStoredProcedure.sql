...
--Get last access of quote with time range filtering and pagination enabled
        ...
        SELECT PolicyId, LastTouchedByTS, QuoteReference, Premium, LastPipelineStatusDescription, PolicyProgressId, TotalRecords
        FROM    PolicyDataRaw
...
SELECT  
       ...
       , ISNULL(ps.CreatedTS, polData.LastTouchedByTS) as LastPipelineStatusTS
       ...
       INNER JOIN PolicyDataRawPaged polData
		...
		INNER JOIN RolledUpPlans polPlans
       ...
       WHERE (@DoPipelineSearch = 0 OR (@DoPipelineSearch = 1 AND ps.CreatedBy = @agent) OR (@DoPipelineSearch = 1 AND polData.PolicyProgressId = 0))
