...
{
    [TestFixture]
    public class PlanOptionAvailabilityProviderTests : IServiceIntegrationTest<PlanOptionAvailabilityProvider>
    {
        ...
        public PlanOptionAvailabilityProvider GetServiceInstance()
        {
            return new PlanOptionAvailabilityProvider(_midMarketProductDefinitionBuilder, 
               new SelectedProductPlanOptionsConverter(), _nameLookupService);
        }

        [Test]
        public void GetAvailableOptions_LifePlan_NoOtherPlanSelected_NoOptionsAvailable()
        {
            ...
            var availableItems = service.GetAvailableOptions(inputModel);
            Assert.That(availableItems.Any(x=> x.IsAvailable), Is.False);
        }

        [Test]
        public void GetAvailableOptions_LifePlan_NoIncomePlanSelected_OptionAvailable()
        {
            var service = GetServiceInstance();
            var inputModel = new AvailabilityPlanStateParam()
            {
            ...
            Assert.That(availableItems.First().Code, Is.EqualTo("PR"));
        }
    }
